...
class QuadratureIntegrator(IntegratorBase):
    ...
    def integrate_along_rays(
        ...
        exponential_term = torch.exp(-sigma * delta)

        alpha = 1 - exponential_term

        transmittance = torch.cat([torch.ones_like(exponential_term[..., :1]), torch.cumprod(exponential_term + 1e-10, dim=-1)], dim=-1)[..., :-1]
